...
// IsSelfTarget reports whether the event landed directly on the listener's
// own element (Target == CurrentTarget), as opposed to bubbling up from a
...
func (e Event) IsSelfTarget() bool { return e.target.Equal(e.currentTarget) }
...
// handler on a container that should only fire for clicks on the container
// itself, not on any button/link inside it.
func (e MouseEvent) IsSelfTarget() bool { return e.target.Equal(e.currentTarget) }
